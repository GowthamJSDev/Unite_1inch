...
import { SimpleLimitOrderDemo, checkAllBalances } from '../helper/simpleLimitOrderDemo';

export default function SimpleLimitOrderDemoComponent() {
  ...
  const loadBalances = async () => {
    try {
      const balanceData = await checkAllBalances(provider, account);
  ...
  const runDemo = async () => {
    ...
    if (parseFloat(balances.eth) < totalEthNeeded) {
      setError(`Insufficient ETH. Need ${totalEthNeeded} ETH (${ethAmount} for swap + 0.01 for gas), have ${balances.eth} ETH`);
      return;
    }
    ...
    try {
      ...
      const demo = new SimpleLimitOrderDemo(provider);
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-lg">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              ETH Amount (will be converted to WETH)
            </label>
            <input
              type="number"
              step="0.001"
            ...
            <p className="text-xs text-gray-500 mt-1">
              Will create order: {ethAmount} WETH → {expectedUsdc.toFixed(2)} USDC
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg">
        <h3 className="font-semibold mb-2 text-blue-800">Demo Flow:</h3>
        <ol className="text-sm text-blue-700 list-decimal list-inside space-y-1">
          <li>Convert ETH to WETH (required for limit orders)</li>
          <li>Create limit order using 1inch SDK</li>
          <li>Sign order with EIP-712 signature</li>
          <li>Execute order onchain (you act as both maker and filler)</li>
          <li>Show transaction results</li>
        </ol>
        <p className="text-xs text-blue-600 mt-2">
          ⚡ This demonstrates the complete 1inch Limit Order Protocol flow
        </p>
      </div>
      ...
      {parseFloat(balances.usdc) < expectedUsdc && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
          <h4 className="font-medium text-yellow-800">⚠️ Insufficient USDC</h4>
          <p className="text-sm text-yellow-700 mt-1">
            You need {expectedUsdc.toFixed(2)} USDC to fill this order, but have {balances.usdc} USDC.
            Consider reducing the ETH amount or getting more USDC.
          </p>
        </div>
      )}
      ...
      {parseFloat(balances.usdc) >= expectedUsdc && parseFloat(balances.eth) > 0.01 && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-md">
          ...
          <p className="text-sm text-green-700 mt-1">
            You have sufficient ETH ({balances.eth}) and USDC ({balances.usdc}) to run the complete 1inch limit order demo.
          </p>
        </div>
      )}
      ...
      <div className="mt-6 p-4 bg-gray-50 rounded-md">
        ...
        <ul className="text-sm text-gray-700 mt-2 space-y-1">
          <li>✅ <strong>Onchain execution</strong>: fillOrder transaction on Ethereum</li>
          <li>✅ <strong>Custom Limit Orders</strong>: Not posted to official 1inch API</li>
          <li>✅ <strong>1inch SDK Integration</strong>: Uses official @1inch/limit-order-sdk</li>
          <li>✅ <strong>EIP-712 Signatures</strong>: Proper order signing</li>
